...
describe('CategoryApiController', () => {
  ...
  describe('create', () => {
    ...
    it('maps duplicate category names to a stable 409 error body', async () => {
      req.body = { nameCategory: 'Existing Category' };
      mockCreateCategoryUseCase.execute.mockRejectedValue(new Error('DUPLICATE_CATEGORY_NAME'));

      await controller.create(req as Request, res as Response, next);

      expect(res.status).toHaveBeenCalledWith(409);
      expect(res.json).toHaveBeenCalledWith({ error: 'DUPLICATE_CATEGORY_NAME' });
      expect(next).not.toHaveBeenCalled();
    });
  });
  ...
  describe('update', () => {
    it('returns 200 and the updated category on success', async () => {
      ...
      expect(mockUpdateCategoryUseCase.execute).toHaveBeenCalledWith(10, {
        nameCategory: 'Updated Name',
      });
      expect(res.json).toHaveBeenCalledWith(mockCategory);
      expect(next).not.toHaveBeenCalled();
    });
    ...
    it('maps duplicate category updates to a stable 409 error body', async () => {
      req.params = { id: '10' };
      req.body = { nameCategory: 'Existing Category' };
      mockUpdateCategoryUseCase.execute.mockRejectedValue(new Error('DUPLICATE_CATEGORY_NAME'));
