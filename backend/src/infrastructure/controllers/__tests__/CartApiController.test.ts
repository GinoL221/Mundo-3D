...
describe('CartApiController', () => {
  ...
  describe('syncCart', () => {
    ...
    it('does not map idProduct to productId and passes items directly', async () => {
      req.body = {
        items: [{ idProduct: 10, quantity: 2 } as any],
      };
      ...
      expect(mockSyncCartUseCase.execute).toHaveBeenCalledWith(5, [{ idProduct: 10, quantity: 2 }]);
      expect(mockGetCartByUserIdUseCase.execute).toHaveBeenCalledWith(5);
      expect(res.json).toHaveBeenCalledWith({
        success: true,
        cart: mockCartResult,
