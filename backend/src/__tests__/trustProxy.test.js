...
describe('trust proxy — proxy-aware runtime and rate limiting', () => {
  ...
  function buildProxyApp({ trustProxy, limiter }) {
    const proxyApp = express();
    if (trustProxy !== undefined) proxyApp.set('trust proxy', trustProxy);
    proxyApp.post('/login', limiter, (req, res) => res.status(200).json({ ip: req.ip }));
    return proxyApp;
  }
  ...
  it('rate-limits each forwarded client IP in its own bucket: one client at the cap does not 429 another', async () => {
    ...
    // Client A exhausts its budget of 2, so its 3rd attempt is refused.
    ...
    expect([a1.status, a2.status, a3.status]).toEqual([200, 200, 429]);
