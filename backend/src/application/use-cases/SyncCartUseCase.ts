...
export class SyncCartUseCase {
  ...
  private mergeItems(items: { productId: number; quantity: number }[]): Map<number, number> {
  ...
  async execute(userId: number, items: { productId: number; quantity: number }[]): Promise<void> {
