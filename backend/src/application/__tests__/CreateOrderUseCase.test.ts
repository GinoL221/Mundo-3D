...
class FakeOrderRepository implements OrderRepositoryPort {
  ...
  async findByUserId(
    ...
    return {
      orders: scoped.slice(options.offset, options.offset + options.limit),
      total: scoped.length,
    };
  }
...
describe('CreateOrderUseCase', () => {
  ...
  it('(a) happy path: creates an AWAITING_PAYMENT order, marks the locked cart rows ORDERED, and attaches the payment reference', async () => {
    ...
    expect(orderRepo.attachPaymentReferenceCalls).toEqual([
      { idOrder: dto.idOrder, reference: `pay-${dto.idOrder}` },
    ]);
