...
} from "../use-cases/UpdateFranchiseUseCase";
...
describe("UpdateFranchiseUseCase", () => {
  ...
  it("returns null when the franchise cannot be updated because it does not exist", async () => {
    ...
    await expect(
      useCase.execute(999, { nameFranchise: "Unknown" }),
    ).resolves.toBeNull();
    expect(franchiseRepo.update).toHaveBeenCalledWith(999, {
      nameFranchise: "Unknown",
    });
  });
});
