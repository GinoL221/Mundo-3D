...
A `render.yaml` MUST exist at the repository root describing the free-tier backend web service. It MUST specify: build command `pnpm --filter backend build`, start command `deploy:start`, environment `RUN_COMPILED=true` and `NODE_ENV=production`, and Node major version 22.

#### Scenario: Manifest fully describes the backend service

- GIVEN the repository
- WHEN `render.yaml` is read
- THEN it MUST define one web service whose build command is `pnpm --filter backend build`, whose start command is `deploy:start`, and whose environment sets `RUN_COMPILED=true`, `NODE_ENV=production`, and Node 22
...
**Change (2026-08-31, object-storage production gate)**: The R2 setup section documents
production-only configuration. Development and test environments use a local-disk fallback
for uploads and do not require R2 credentials to be set.
