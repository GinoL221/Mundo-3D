...
- [x] Implement login page `login.astro` and register page `register.astro` with JWT storing in `localStorage`.
- [x] Fetch products list in frontend pages dynamically using client-side fetching from Express backend.
- [x] Build reactive cart store in `frontend/src/store/cart.ts` via Nano Stores syncing with API.

## Phase 6: EJS Views and MVC Controller Removal
- [x] Delete legacy controllers: `UserController.ts`, `ProductController.ts`, `CartController.ts`, `StaticPagesController.ts`.
- [x] Remove unused routes: `staticPagesRoutes.ts`, `productRoutes.ts`, `userRoutes.ts`, `cartRoutes.ts`.
- [x] Delete EJS files under `src/views/` and run final integration/build verification checks.
